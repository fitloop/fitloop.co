EmbedInstagram = function() {


    /*1427744936,,JIT Construction: v1666213,en_US*/
    ...
    try {
        window.instgrm || (function(window) {
            ...
            __d("CSSCore", ["invariant"], function(a, b, c, d, e, f, g) {
                ...
                var h = {
                    ...
                    hasClass: function(i, j) {
                        g(!/\s/.test(j));
                        if (i.classList) return !!j && ES(i.classList, 'contains', true, j);
            ...
            __d("ig.sdk.Embeds", ["CSS", "DOMEventListener", "sdk.domReady", "IGIframeableMessageTypes", "invariant", "isNumberLike", "toArray"], function(a, b, c, d, e, f, g, h, i, j, k, l, m) {
                ...
                var n = g,
                    o = n.addClass,
                    p = n.hasClass,
                    q = ['instagram\\.com', 'instagr\\.am'],
                    r = 'data-instgrm-captioned',
                    s = 'instagram-embed-',
                    ...
                    y = x + '-registered',
                    z = x + '-rendered',
                    aa = new RegExp('^' + 'https?://' + '([\\w-]+\\.)*(' + q.join('|') + ')' + '$'),
                    ba = 'data-instgrm-payload-id',
                    ca = 'instagram-media-payload-',
                    da = new RegExp('^(' + aa.source.replace(/^\^/, '').replace(/\$$/, '') + '/p/[^/]+' + ')'),
                    ea = 'data-instgrm-preserve-position',
                    fa = 'data-instgrm-version',
                    ga = {},
                    ha = false,
                    ia = 0,
                    ja = false;
                ...
                function ka(qa) {
                    var ra = document.getElementsByTagName('iframe'),
                        sa;
                ...
                function ma(qa) {
                    var ra = qa.getElementsByTagName('a'),
                        sa = ra.length;
                ...
                function na(qa, ra) {
                    var sa = ia++,
                        ta = s + sa;
                    if (!qa.id) qa.id = ca + sa;
                    var ua = ra.replace(w, '/$1');
                    ua += 'embed/';
                    if (qa.hasAttribute(r)) ua += 'captioned/';
                ...
                function oa(qa) {
                    ...
                    if (typeof ta !== 'object' || typeof ta.type !== 'string' || typeof ta.details !== 'object') return;
                    var va = ta,
                        wa = va.details,
                        xa = va.type,
                        ya = null;
                ...
                function pa() {
                    ...
                    i(ES(function() {
                        ...
                        ES(m(qa), 'forEach', true, function(ra) {
                            if (ra.tagName !== 'BLOCKQUOTE' || p(ra, y)) return;
                            var sa = ma(ra);
                            if (sa) na(ra, sa);
                        });
                        if (!ha) {
        ...
        new Image().src = "https:\/\/www.facebook.com\/" + 'common/scribe_endpoint.php?c=jssdk_error&m=' + encodeURIComponent('{"error":"LOAD", "extra": {"name":"' + e.name + '","line":"' + (e.lineNumber || e.line) + '","script":"' + (e.fileName || e.sourceURL || e.script) + '","stack":"' + (e.stackTrace || e.stack) + '","revision":"1666213","message":"' + e.message + '"}}');
    }
